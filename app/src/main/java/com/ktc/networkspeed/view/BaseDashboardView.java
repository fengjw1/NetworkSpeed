...
public abstract class BaseDashboardView extends View {
    ...
    //大小刻度的数量
    protected int mLargeCalibrationNumber;//大刻度
    protected int mLargeBetweenCalibrationNumber;//小刻度
    //刻度总数
    ...
    //刻度文本
    protected float[] mCalibrationNumberText;
...
//        mCalibrationBetweenText = DEFAULT_CALIBRATION_BETWEEN;
        mMin = (int) mCalibrationNumberText[0];
        mMax = (int) mCalibrationNumberText[mCalibrationNumberText.length - 1];
    ...
    public void setValue(float value, boolean isAnim, boolean reset) {
        value = value < mMin ? mMin : value > mMax ? mMax : value; //这句话是说只能转动mMin-mMax之间的数值
        //计算进度需要转动的角度
...
//        }
     ...
     * 设置刻度信息
     */
    public void setCalibration(float[] calibrationNumberText, String[] calibrationBetweenText, int largeCalibrationBetweenNumber){
...
//        mCalibrationBetweenText = calibrationBetweenText;
    ...
    protected int dp2px(float dpValue) {
        ...
        return (int) (dpValue * density + 0.5f);//为了精确度，需要加0.5f【float转int】
    }
    ...
    protected abstract void drawText(Canvas canvas, float value, String valueLevel, String currentTime);
}
